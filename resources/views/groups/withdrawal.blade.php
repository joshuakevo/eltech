@extends('layouts.app')
@section('title', 'Group Withdrawal')
@section('breadcrumb')
...
@endsection
@section('content')
<div class="mb-4"><h4 class="fw-bold mb-0">Post Withdrawal — {{ $group->name }}</h4></div>

<div class="card" style="max-width:640px">
    <div class="card-body">
        <form method="POST" action="{{ route('groups.withdraw.store', $group) }}">
            @csrf
            <div class="mb-3">
                <label class="form-label fw-semibold">Posting Mode <span class="text-danger">*</span></label>
                <select name="mode" id="wMode" class="form-select" onchange="updateWMode()">
                    ...
                    <option value="custom"      {{ old('mode')==='custom'                 ?'selected':'' }}>Group-wide — custom per member</option>
                </select>
            </div>
            ...
            <div id="customRow" class="d-none mb-3">
                ...
                <table class="table table-sm table-bordered mb-0">
                    <thead class="table-light"><tr><th>Member</th><th class="text-end">Balance</th><th>Withdraw (UGX)</th></tr></thead>
                    <tbody>
                    @foreach($members as $m)
                    <tr>
            ...
            <div class="d-flex gap-2">
                <button type="submit" class="btn btn-danger">Post Withdrawal</button>
