...
<div class="header clearfix">
    ...
    <h1>@php $_logo = \App\Models\SystemSetting::get('org_logo'); @endphp@if($_logo)<img src="{{ public_path('storage/'.$_logo) }}" style="height:32px;max-width:160px;object-fit:contain;vertical-align:middle">@else{{ \App\Models\SystemSetting::get('org_name', 'ElTech Finance') }}@endif — Savings Account Balances</h1>
    <p>All active savings accounts</p>
</div>

<div class="total-box">
    <div class="lbl">Total Savings Balance</div>
...
<table>
    ...
    <tr>
        <td style="font-family:monospace;font-size:8px">{{ $acc->account_number }}</td>
        <td>{{ $acc->client->name }}</td>
        <td style="font-size:8px;color:#6b7280">{{ $acc->product->name }}</td>
        <td class="r" style="font-weight:bold;color:#2563eb">{{ number_format($acc->balance, 2) }}</td>
        <td style="font-size:8px">{{ ucfirst($acc->status) }}</td>
    </tr>
    @empty
    <tr><td colspan="5" style="text-align:center;color:#9ca3af;padding:10px">No savings accounts found.</td></tr>
    @endforelse
    </tbody>
    <tfoot>
        <tr>
            <td colspan="3">TOTAL ({{ $accounts->count() }} accounts)</td>
            <td class="r" style="color:#2563eb">{{ number_format($total, 2) }}</td>
            <td></td>
        </tr>
    </tfoot>
</table>
