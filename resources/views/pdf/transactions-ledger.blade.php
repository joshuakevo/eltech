...
<style>
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    tr.bf td { background:#eef2f7; font-weight:bold; }
...
<table>
    ...
    <tbody>
        <tr class="bf">
            ...
            <td class="r">{{ number_format($openingBalance, 0) }}</td>
        </tr>
        ...
        <tr>
            ...
            <td class="r {{ $row['balance'] < 0 ? 'text-danger' : '' }}">{{ number_format($row['balance'], 0) }}</td>
        </tr>
        @empty
        <tr><td colspan="6" style="text-align:center;color:#9ca3af;padding:10px">No transactions in this range.</td></tr>
        @endforelse
        <tr class="bf">
            <td colspan="5">Closing Balance</td>
            <td class="r">{{ number_format($ledgerRows->last()['balance'] ?? $openingBalance, 0) }}</td>
        </tr>
    </tbody>
</table>
<div class="footer">ElTech Finance &bull; Journal Entries export &bull; Printed by {{ auth()->user()->name ?? 'System' }}</div>
</body>
</html>
