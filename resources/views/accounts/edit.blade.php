...
<div class="row justify-content-center">
    <div class="col-md-7">
        <div class="card">
            <div class="card-header">Edit Account — {{ $account->account_code }}</div>
            <div class="card-body">
                <form method="POST" action="{{ route('accounts.update', $account) }}">
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-4">
                            ...
                            <input type="text" name="account_code" class="form-control font-monospace @error('account_code') is-invalid @enderror" value="{{ old('account_code', $account->account_code) }}" required>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="parent_id" class="form-select">
                                ...
                                @foreach($parents as $p)
                                    <option value="{{ $p->id }}" @selected(old('parent_id',$account->parent_id)==$p->id)>{{ $p->account_code }} — {{ $p->account_name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
