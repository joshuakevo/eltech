 ...
 * on purpose. Three loans from the report were deliberately left out
...
class FixJuly2026LoanTerms extends Command
{
    ...
    protected function fixLoan(array $row): void
    {
        ...
        $this->line("Fixed {$row['client_number']} (loan #{$row['loan_id']}): {$row['interest_rate']}% {$row['interest_method']}, {$row['term_months']}mo, disbursed {$row['disbursement_date']}");
    }
}
